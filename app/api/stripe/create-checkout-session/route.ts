...
export async function POST(request: NextRequest) {
  try {
    ...
    const { priceId, subscriptionTier, type } = body
    ...
    if (type === "coins") {
      ...
      checkoutSession = await stripe.checkout.sessions.create({
        ...
        mode: "payment",
        success_url: `${request.headers.get('origin') || process.env.NEXTAUTH_URL}/subscription?success=true&type=coins`,
      ...
      checkoutSession = await stripe.checkout.sessions.create({
        ...
        mode: "subscription",
        success_url: `${request.headers.get('origin') || process.env.NEXTAUTH_URL}/subscription?success=true&tier=${subscriptionTier}`,
        cancel_url: `${request.headers.get('origin') || process.env.NEXTAUTH_URL}/subscription?canceled=true`,
        metadata: {
          userId: user.id,
          subscriptionTier,
